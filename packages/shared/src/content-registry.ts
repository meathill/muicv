...
ATS 的核心处理流水线分为四个关键步骤：
- **步骤 1：文本抽取 (Text Extraction)**：ATS 会将 PDF 还原为纯文本流。如果你的简历使用了复杂的 Canvas 渲染、浮动文本框、不可复制的图片文字，抽取出来的就会是一堆乱码或空白！
- **步骤 2：段落与实体识别 (Section Parsing)**：算法会寻找标准命名，如 *Work Experience*, *Education*, *Skills*, *Projects*。如果使用了非标小标题（如「我的极客之旅」「曾经战斗过的地方」），系统就无法归类你的工作经历。
- **步骤 3：关键词语义加权匹配 (Keyword Scoring)**：系统将你提取出的技能与目标职位的 JD 进行匹配打分。
- **步骤 4：生成匹配度打分 (Rank Score)**：只有 Rank 处于前 15%~20% 的候选人，HR 才会点开人工查看。
